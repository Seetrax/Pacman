...
def sq(x1,x2):
    return (abs(x1[0] - x2[0])**2 + abs(x1[1] - x2[1])**2)**0.5
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    min=mh(state[0],corners[0])
    if state[1:5]==(1,1,1,1):
        return 0
    for i in corners:
        if min>mh(state[0],i):
            min=mh(state[0],i)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if l1==[]:
        return 0
    min=max=mh(position,l1[0])
    sum=0
    for k in l1:
        sum+=mh(position,k)
        if max<mh(position,k):
            max=mh(position,k)
        if min>mh(position,k):
            min=mh(position,k)
    return max
    return 0
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        for i in range (food.width):
            for j in range (food.height):
                if food[i][j]==True:
                    l1.append((i,j))
        min2=sq(startPosition,l1[0])
        mins2=l1[0]
        for k in l1:
            if min2>sq(startPosition,k):
                min2=sq(startPosition,k)
                mins2=k
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        for i in range (self.food.width):
            for j in range (self.food.height):
                if self.food[i][j]==True:
                    l1.append((i,j))
        min3=sq(self.startState,l1[0])
        mins3=l1[0]
        for k in l1:
            
            if min3>sq(self.startState,k):
                min3=sq(self.startState,k)
                mins3=k
        return state==mins3
