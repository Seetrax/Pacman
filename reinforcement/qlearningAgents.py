...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        maxa=-99999999
        for i in act:
            if maxa<self.getQValue(state,i):
                maxa=self.getQValue(state,i)
        return maxa
    ...
    def computeActionFromQValues(self, state):
        ...
        maxa=None
        for i in act:
            if maxi<self.getQValue(state,i):
                maxi=self.getQValue(state,i)
                maxa=i
        return maxa
        "*** YOUR CODE HERE ***"
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.transition[(state,action)]=(nextState)
        util.raiseNotDefined()'''
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        ff=self.featExtractor.getFeatures(state,action)
        actions=self.getLegalActions(nextState)
        
        difference=reward+self.discount*(self.getValue(nextState))-self.getQValue(state,action)
        for i in ff:
            self.weights[i]+=self.alpha*(difference)*ff[i]   
