...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in variableDomainsDict[PAC]:
        for j in variableDomainsDict[GHOST0]:
        ...
        for j in variableDomainsDict[GHOST0]:
            if (manhattanDistance(i,j))-MAX_NOISE<0:
                ...
                for l in range((manhattanDistance(i,j))-MAX_NOISE,(manhattanDistance(i,j))+MAX_NOISE+1):
                    if l not in lis1:
                        lis1.append(l)
    variableDomainsDict[OBS0]=lis0
    variableDomainsDict[OBS1]=lis1
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        while eliminationOrder!=[]:
            u=eliminationOrder.pop(0)
            cpts,joined=joinFactorsByVariable(cpts,u)
            if len(joined.unconditionedVariables())!=1:
                new=eliminate(joined,u)
                cpts.append(new)
        new=joinFactors(cpts)
        return normalize(new)
        '''for i in evidenceVariablesSet:
            tup2=joinFactorsByVariable(cpts,i)
            cpts=tup2[0]
            cpts.append(tup[1])
        print(len(cpts))'''
        return cpts[0]
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        l1=list(self.values())
        l2=list(self.keys())
        return random.choices(l2, weights = l1, k = 1)[0]
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance==None:
            return 0
        p=busters.getObservationProbability(noisyDistance, dis)
        return p
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.allPositions:
            self.beliefs[i]*=self.getObservationProb(observation,pac_pos,i,jail)
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
