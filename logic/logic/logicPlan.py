...
def sentence1() -> Expr:
    ...
    F1=Expr('|',Expr('A'),Expr('B'))
    F2=Expr('<=>',Expr('~',Expr('A')),Expr('|',Expr('~',Expr('B')),Expr('C')))
    F3=Expr('|',Expr('~',Expr('A')),Expr('~',Expr('B')),Expr('C'))
    return Expr('&',F1,F2,F3)
...
def sentence2() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    F1=Expr('<=>',Expr('C'),Expr('|',Expr('B'),Expr('D')))
    F2=Expr('>>',Expr('A'),Expr('&',Expr('~',Expr('B')),Expr('~',Expr('D'))))
    F3=Expr('>>',Expr('~',Expr('&',Expr('B'),Expr('~',Expr('C')))),Expr('A'))
    F4=Expr('>>',Expr('~',Expr('D')),Expr('C'))
    return Expr('&',F1,F2,F3,F4)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    F1=Expr('|',Expr('&',PacmanAlive_0,Expr('~',PacmanKilled_0)),Expr('&',Expr('~',PacmanAlive_0),PacmanBorn_0))
    F2= Expr('<=>',PacmanAlive_1,F1)
    F3=Expr('~',Expr('&',PacmanAlive_0,PacmanBorn_0))
    F4=PacmanBorn_0
    return Expr('&',F2,F3,F4)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***" # might be helpful for getting ideas
    a=Expr('A')
    a.op='a'
    return {a:True}
    util.raiseNotDefined()
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    not_inv=Expr('~',inverse_statement)
    return pl_true(not_inv,assignments)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    ll=list(itertools.combinations(literals,2))
    arr=[]
    for i in ll:
        arr.append(Expr('|',Expr('~',i[0]),Expr('~',i[1])))
    return conjoin(arr)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    if not possible_causes:
        return None
    e=conjoin([~PropSymbolExpr(pacman_str, x, y, time=last) , ~PropSymbolExpr(wall_str, x, y), disjoin(possible_causes)])
    return Expr('<=>',PropSymbolExpr(pacman_str, x, y, time=now),e )
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
